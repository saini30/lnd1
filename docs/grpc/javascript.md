...
let lightning = new lnrpc.Lightning('localhost:10006', credentials);
```
...
of which is at the default `localhost:10006`, with an open channel between the
two nodes.
...
Now, create an invoice for your node at `localhost:10006`and send a payment to
...
let lnrpcDescriptor = grpc.loadPackageDefinition(packageDefinition);
let lnrpc = lnrpcDescriptor.lnrpc;
let client = new lnrpc.Lightning('some.address:10006', credentials);
